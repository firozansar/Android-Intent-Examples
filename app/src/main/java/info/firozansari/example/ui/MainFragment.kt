...
import info.firozansari.example.utils.DemoItem
import info.firozansari.example.utils.EmailIntents
import info.firozansari.example.utils.GeoIntents
import info.firozansari.example.utils.ItemAdapter
import info.firozansari.example.utils.MediaIntents
import info.firozansari.example.utils.PhoneIntents
import info.firozansari.example.utils.ShareIntents
import info.firozansari.example.utils.SystemIntents
import java.util.*
...
class MainFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        textView.setOnClickListener {
            activity?.let { it1 ->
                AlertDialog.Builder(it1).setMessage(intentExplanationText).show()
            }
        }
    ...
    private val demoItems: List<DemoItem>
        private get() {
            ...
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.dialer),
                    PhoneIntents.newDialNumberIntent(null)
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.call_number),
                    PhoneIntents.newCallNumberIntent("07957941679")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.dial_number),
                    PhoneIntents.newDialNumberIntent("07957941679")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.send_sms_to),
                    PhoneIntents.newSmsIntent(activity, "this is a test SMS", "07957941679")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.send_sms),
                    PhoneIntents.newSmsIntent(activity, "this is a test SMS")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.pick_contact),
            ...
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.show_alarms),
                    PhoneIntents.showAllAlarms()
                )
            )
            ...
            demoItemList.add(
                DemoItem(
                    ...
                    GeoIntents.newMapsIntent(53.600910f, -2.547780f, "Horwich, Bolton")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.navigate_to_address),
                    GeoIntents.newNavigationIntent("Manchester Piccadilly Garden")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.navigate_to_location),
                    GeoIntents.newNavigationIntent(53.600910f, -2.547780f)
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.streetview_at_location),
                    GeoIntents.newStreetViewIntent(53.600910f, -2.547780f)
                )
            )
            ...
            demoItemList.add(
                DemoItem(
                    ...
                    MediaIntents.newPlayImageIntent(
                        "http://upload.wikimedia.org/wikipedia/commons/thumb/a/a9/Biarritz-Plage.JPG/1920px-Biarritz-Plage.JPG")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.play_audio),
                    MediaIntents.newPlayAudioIntent(
            ...
            demoItemList.add(
                DemoItem(
                    ...
                    MediaIntents.newPlayVideoIntent(
                        "http://mirror.bigbuckbunny.de/peach/bigbuckbunny_movies/big_buck_bunny_480p_h264.mov")
                )
            )
            // demoItemList.add(new DemoItem(res.getString(R.string.play_video_youtube),
            // MediaIntents.newPlayYouTubeVideoIntent("b_yiWIXBI7o")));
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.browse_web),
                    MediaIntents.newOpenWebBrowserIntent("http://firozansari.info")
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.take_pic),
                    MediaIntents.newTakePictureIntent()
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.select_pic),
            ...
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.share), ShareIntents.newShareTextIntent(
                        "My subject", "My message", getString(
            ...
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.show_wifi_settings),
                    SystemIntents.showWifiSettings()
                )
            )
            demoItemList.add(
                DemoItem(
                    res.getString(R.string.show_nfc_settings),
                    SystemIntents.showNfcSettings()
                )
            )
